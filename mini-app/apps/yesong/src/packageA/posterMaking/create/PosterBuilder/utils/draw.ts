...
export function _drawRadiusGroupRect({ x, y, w, h, g }, { ctx }) {
  const [borderTopLeftRadius, borderTopRightRadius, borderBottomRightRadius, borderBottomLeftRadius] = g
  ctx.beginPath()
...
export function drawText(params, drawOptions) {
  ...
  if (Object.prototype.toString.call(text) === '[object Array]') {
    ...
    text.forEach(item => {
      ...
      // TODO:多段文字超出一行的处理
      const textWidth = _drawSingleText(Object.assign(item, { ...preText, y: y + (item.marginTop || 0) }), drawOptions)
