<!--
需要借助第三方的神秘力量完成此功能
github地址:https://github.com/BibBin/taro-poster-builder
-->
<template>
  <canvas
    type="2d"
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    tempConfig.images = tempConfig.images.filter(item => {
      return item.url
    })
    const { width, height, backgroundColor, texts = [], blocks = [], lines = [], debug = false } = tempConfig || {}
    ...
    const initImages = (images: Image[]) => {
      const imagesTemp = images.filter(item => item.url || item?.base64Url)
      const drawList = imagesTemp.map((item, index) => {
        return getImageInfo(item, index)
      })
    ...
    const init = () => {
      if (props.showLoading) {
        Taro.showToast({ mask: true, icon: 'none ', title: '生成中...' })
      }
