...
export const firstSwiper = computed(() => {
  ...
  const cs = useCanvasStore() as any
  const nodes = cs?.layerTree?.menuNodes
  if (!Array.isArray(nodes)) return undefined
  const node = nodes.find(
    (item: any) =>
      item?.depth === 0 && item.$component?.key === 'swiper' && !(item.hidden || item.inheritHidden)
  )
  if (typeof window !== 'undefined' && localStorage.getItem('swiperDebug') === '1') {
    try {
      console.log('[manifest(render)] firstSwiper candidates', nodes
        .filter((n: any) => n?.$component?.key === 'swiper')
        .map((n: any) => ({ id: n.id, depth: n.depth, hidden: !!n.hidden, inheritHidden: !!n.inheritHidden })))
      console.log('[manifest(render)] firstSwiper pick', node?.id)
    } catch {}
  }
  return node
})
...
export const isNavDynamicForeground = computed(
  () => {
    const cs = useCanvasStore() as any
    return cs?.page?.navigator?.foreground?.dynamicColor
  }
)
