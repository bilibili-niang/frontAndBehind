...
try {
  TARO_APP_NO_WECHAT_PAY = process.env.TARO_APP_NO_WECHAT_PAY === 'true'
} catch (err) {}
  ...
  getPayParams(orderNo)
    .then(res => {
      ...
      if (res.code === 200) {
        ...
        const wechatPay = TARO_APP_NO_WECHAT_PAY
          ? () => {
              useToast('无法使用微信支付')
...
const navigateToUnionPay = (
  ...
  if (process.env.TARO_ENV === 'h5') {
    useOpenWeapp({
      ...
      appName: '云闪付',
      appLogo: 'https://dev-cdn.null.cn/upload/e248d3a7a6a15c0e311afebd647275a8.svg',
      success: () => {
        options?.success?.()
      },
      fail: err => {
