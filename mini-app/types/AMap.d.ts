declare module AMap {
  ...
  export class MapsEvent {
    /** 发生事件时光标所在处的经纬度坐标
     * noWrap: 是否自动将经度值修正到 [-180,180] 区间内，缺省为false; noWrap 为false时传入 [190,30] ，会被自动修正为 [-170,30] , noWrap 为true时不会自动修正，可以用来进行跨日期限的覆盖物绘制
     **/
    public lnglat: { lng: number; lat: number; noWrap: boolean }
  ...
  export class LngLat {
    ...
    /**设置经度值 参数说明： lng (number) 经度*/
    public setLng(lng)

    /**设置纬度值 参数说明： lat (number) 经度*/
    public setLat(lat)

    /**与另外一个经纬度相加
     * another (LngLat) 另外一个经纬度坐标
     * noWrap (boolean) 是否将相加的结果经度值修正到 [-180,180] 区间内*/
    public add(another, noWrap): LngLat

    /**与另外一个经纬度相减
     * another (LngLat) 另外一个经纬度坐标
     * noWrap (boolean) 是否将相加的结果经度值修正到 [-180,180] 区间内*/
    public subtract(another, noWrap): LngLat
  ...
  export class Map {
    ...
    public add(overlayers: VectorOverlay | Array<any>): void
  ...
  export class Marker {
    ...
    /**设置点标记位置**/
    public setPosition(lnglat: LngLat | number[]): void
    ...
    public moveTo(lnglat: LngLat, opts?: MoveToOptions): void
  ...
  export interface LabelMarkerOptions {
    ...
    zIndex?: number //		同一 LabelsLayer 内标注显示层级，数字越大越靠前，默认值: 1
    visible?: boolean //		标注是否可见， 默认值: true
    extData?: any //		用户自定义类型数据，可将自定义数据保存在该属性上，方便后续操作使用。
    icon?: IconOptions //		标注图标设置
    text?: TextOptions //		标注文本设置
  }
  ...
  export interface LabelsLayerOptions {
    visible?: boolean //	标注层是否可见，默认值:true
    zIndex?: number //	标注层与其它图层的叠加顺序，默认值：120
    opacity?: number //	标注层透明度
    collision?: boolean //	标注层内的标注是否避让
    allowCollision?: boolean //	标注层内的标注是否允许其它标注层对它避让
    zooms?: [number, number] //	标注层展示层级范围
  }
  ...
  /**index (number) 运行到点的索引, data (LngLat) 当前点经纬度**/
  ...
  export interface TextStyleOptions {
    ...
    padding?: string | Array<string | number> //	文字 padding。默认值： [3, 3, 3, 3]
    strokeWidth?: number
  ...
  export class Polygon {
    ...
    public setOptions(opt: PolygonOptions): void
  ...
  export interface CircleOptions {
    // 要显示该circle的地图对象 （自v1.2 新增）
    map?: AMap.Map
    // 层叠顺序, 默认zIndex:10
    zIndex?: number
    // 圆心位置
    center?: AMap.LngLat | number[]
  ...
  export class Circle {
    ...
    /**
     *  hide 隐藏 属性说明： data ({type: string, target: Object})
     *  show 显示 属性说明： data ({type: string, target: Object})
     *  click 鼠标左键单击事件 属性说明： data (MapsEvent)
     *  dblclick 鼠标左键双击事件 属性说明： data (MapsEvent)
     *  rightclick 鼠标右键单击事件 属性说明： data (MapsEvent)
     *  mousedown 鼠标按下 属性说明： data (MapsEvent)
     *  mouseup 鼠标抬起 属性说明： data (MapsEvent)
     *  mouseover 鼠标经过 属性说明： data (MapsEvent)
     *  mouseout 鼠标移出 属性说明： data (MapsEvent)
     *  touchstart 触摸开始时触发事件，仅适用移动设备 属性说明： data (MapsEvent)
     *  touchmove 触摸移动进行中时触发事件，仅适用移动设备 属性说明： data (MapsEvent)
  ...
  export class CircleEditor {
     ...
     * 注册事件
     * @param event
     * (addnode:增加一个节点时触发此事件, 回调：data ({target: Circle, lnglat: LngLat, pixel: Pixel}))
     * (adjust:调整折线上某个点时触发此事件, 回调：data ({target: Circle, lnglat: LngLat, pixel: Pixel}))
     * (move:移动覆盖物时触发此事件, 回调：data ({target: Circle, lnglat: LngLat, pixel: Pixel}))
     * (add:创建一个覆盖物之后触发该事件，target即为创建对象。当editor编辑对象为空时，调用open接口，再点击一次屏幕就会创建新的覆盖物对象, 回调：data ({target: Circle}))
  ...
  /**
   * addnode: data ({target: Polygon, lnglat: Lnglat, pixel: Pixel})
   * removenode: data ({target: Polygon, lnglat: Lnglat, pixel: Pixel})
  ...
  export class PolygonEditor {
    ...
    // 添加吸附多边形
    addAdsorbPolygons(list: Polygon | Array<Polygon>)

    // 删除吸附多边形
    removeAdsorbPolygons(list: Polygon | Array<Polygon>)
