...
const metaModules = import.meta.glob('../views/**/index.{tsx,vue}', { eager: true, import: 'routeMeta' }) as Record<string, RouteMeta>
...
const children: RouteRecordRaw[] = Object.keys(viewModules).map((key) => {
...
// 菜单数据：过滤掉 hideInMenu 的路由
export const menuRoutes: RouteRecordRaw[] = enhancedChildren.filter(r => !(r.meta as any)?.hideInMenu)
