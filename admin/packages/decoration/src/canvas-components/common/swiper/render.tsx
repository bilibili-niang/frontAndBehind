...
import { Empty, Icon } from '@anteng/ui'
import { ActionImageDefine } from '../../../widgets/action-image'
import { ActionImage } from '../image/render'
import { emitter, uuid } from '@anteng/core'
import { firstSwiper, isNavDynamicForeground } from './manifest'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const swiperStyle = computed(() => {
      const { padding = [0, 0, 0, 0], ratio = { width: 8, height: 5 } } = props.comp.config
    ...
    const indocatorStyle = computed(() => {
      ...
      const padding = props.comp.config.padding ?? [0, 0, 0, 0]
    ...
    const imageStyle = computed(() => {
      return {
        width: '100%',
        height: '100%',
        borderRadius: props.comp.config.borderRadius.map((i) => withUnit(i)).join(' '),
        objectFit: props.comp.config.ratio.objectFit,
        background: props.comp.config.background
      }
    })
    ...
    const isFirstSwiper = computed(() => {
      ...
      if (swiperDebug) {
        console.log('[swiper(canvas)] isFirstSwiper', {
    ...
    watch(
      ...
      (val) => {
        if (swiperDebug) {
          console.log('[swiper(canvas)] theme change', {
        ...
        if (isFirstSwiper.value) {
          ...
          if (swiperDebug) {
            console.log('[swiper(canvas)] emit', `nav-swiper-theme-color-${currentPageId.value}`, val)
          }
        }
      },
      { immediate: true }
